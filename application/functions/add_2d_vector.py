...
def add_2d_vector(x_coordinates, y_coordinates, old_fig) -> go.Figure:
    ...
    new_fig.update_layout(
        shapes = old_fig.get('layout', []).get('shapes', []) + [
            dict(type='path', path=f'M {tri_x1} {tri_y1} L {tri_x2} {tri_y2} L {tri_x3} {tri_y3} Z', fillcolor='rgb(255,0,0)', line_color='rgb(255,0,0)'),
            dict(type='line', x0=x0, y0=y0, x1=x1-((x1-x0)/90), y1=y1-((y1-y0)/90), line=dict(color="rgb(255,0,0)"))
        ] 
    )
